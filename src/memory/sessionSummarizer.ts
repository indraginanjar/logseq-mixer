...
export async function summarizeSession(messages: Array<{ role: string; content: string }>, settings: any): Promise<string | null> {
  try {
    ...
    const result = await queryLiteLLM(llmMessages, settings.selectedModel, resolveApiKey(settings), resolveChatEndpoint(settings), undefined, undefined, settings.chatProvider, settings.reasoningEffort);
