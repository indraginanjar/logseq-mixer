...
- To embed an image in a block, use standard markdown image syntax: "![description](url)". Valid sources include: external URLs ("![photo](https://example.com/img.png)") or existing graph assets ("![photo](../assets/image.png)"). If the user attaches an image in chat, do NOT write base64 data URIs into blocks (they are unreadable in Logseq). Instead, insert the block with a text placeholder like "![attached image]()" and the system will handle pasting the image via Logseq's native mechanism.`;
}
