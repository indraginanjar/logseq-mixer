...
- To embed an image in a block, use standard markdown image syntax: "![description](url)". Valid sources include: external URLs ("![photo](https://example.com/img.png)") or existing graph assets ("![photo](../assets/image.png)"). If the user attaches an image in chat, do NOT write any image placeholder or data URI into blocks. Simply respond with text acknowledging the request — the system will provide copy-paste instructions to the user separately.`;
}
