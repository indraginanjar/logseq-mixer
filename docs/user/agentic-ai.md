...
**Filtering:** Single-step write/edit requests (e.g., "write a block", "add a note", "insert a TODO") are filtered out — they go to Allow Graph Edits instead of the agent, even if they match goal patterns.
...
> **Note:** Write tools (insert, update, create page) are only available when Allow Graph Edits (✏️) is ON. Read-only tools (search, get page, get blocks) are always available.
